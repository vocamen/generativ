...
function widthSort() {
	for (var x = 0; x < width - 1; x+=1) {
		for (var y = 0; y < height - 1; y+=1) {
			var index = x + y * width;
//			var nx = Math.round(Math.sqrt(Math.pow(x, 2) + Math.pow(y, 2)));
//			var ny = Math.round(Math.atan(y/(x)) * height/2);
			
			//var nx = Math.round(x * 100000 / (Math.pow(x, 2) + Math.pow(y, 2.2)));
			//var ny = Math.round(y * 100000 / (Math.pow(x, 2) + Math.pow(y, 2)));
			
			var nx = x;//Math.round(Math.sin(6.283 * 0.2 * x / width) * width);
			...
			var newIndex = Math.round(nx + ny * width);
			
			newCellData[newIndex] = oldCellData[index];
